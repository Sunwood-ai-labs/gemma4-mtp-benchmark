...
`--max-new-tokens 512` の長め `mt-bench` も確認しました。この M1 Max では長くしても Gemma 4 31B は改善せず、baseline `8.87 tok/s` に対して DFlash `4.87 tok/s`、`0.55x` でした。Qwen3.5-4B も長め `mt-bench` ではほぼ横ばいで、baseline `23.13 tok/s`、DFlash `22.51 tok/s`、`0.97x` です。

DFlash が効く条件と効かない条件を見たい場合は、contrast 実験を使います。
