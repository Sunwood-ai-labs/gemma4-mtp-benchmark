...
CASES = [
    Case(
        "cli_argparse",
        "Write only Python code. Create a CLI tool that accepts --input CSV, --group-by "
        "column, and --top N. It should read the CSV with the standard library, group rows "
        "by the selected column, count rows per group, sort descending, print the top N "
        "groups, and handle missing files or columns with clear errors.",
    ),
    Case(
        "pytest_tests",
        "Write only Python pytest code. Given a function slugify(text: str) -> str that "
        "lowercases text, trims whitespace, replaces non-alphanumeric runs with hyphens, "
        "and strips leading/trailing hyphens, create comprehensive tests for normal text, "
        "Japanese text, repeated separators, empty input, and idempotency.",
    ),
    Case(
        "unified_diff_fix",
        "Return only a unified diff. Fix this Python bug: parse_port(value) currently "
        "returns int(value) without validation. It should reject non-integers, values below "
        "1, and values above 65535 by raising ValueError with a helpful message. Include "
        "unit tests in tests/test_parse_port.py.",
    ),
    Case(
        "open_architecture",
        "Design a small plugin architecture for a Python note-taking app. Include the "
        "interfaces, lifecycle hooks, security considerations, example plugins, and the "
        "main tradeoffs. Use prose and code where helpful.",
    ),
]
...
def main() -> int:
    ...
    max_tokens = int(os.environ.get("DFLASH_MAX_NEW_TOKENS", "320"))
